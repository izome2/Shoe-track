...
const FlexContent = ({ ifExists, endpoint: { title, heading, text, img, } }) => {
  ...
  return (
    <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse sm:flex-col' : 'flex-row sm:flex-col'} mt-8 sm:mt-12`}
      ref={ref}
    >
      <div className='relative max-w-lg lg:max-w-none w-full md:text-center sm:text-center grid items-center lg:justify-items-center'>
        ...
        <motion.div className='relative z-10 p-4 sm:p-2'
        ...
        >
          ...
          <motion.p className='xl:text-sm sm:text-xs my-4 text-white drop-shadow-lg'
            initial={{ opacity: 0, y: 20 }}
            animate={controls}
            transition={{ duration: 0.6 }}
          >
      ...
      <div className='flex items-center justify-center max-w-xl relative lg:max-w-none w-full'>
          <img
            src={img}
            alt={`img/${heading}`}
            className={`object-cover transitions-theme ${ifExists ? 'rotate-[-20deg] hover:-rotate-12' : 'rotate-[19deg] hover:rotate-12'} sm:rotate-0 mt-8 sm:mt-12`} 
          />
      </div>
    </div>
  );
